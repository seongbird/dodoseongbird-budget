...
function remoteSave(){
  if(!apiConfigured() || syncing) return;
  syncing=true; setSyncStatus('Google Sheets 저장 중…');
  fetch(API_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'replaceState',payload:state})})
    .then(()=>{setSyncStatus('Google Sheets 저장됨'); setTimeout(()=>remoteLoad(),500);})
...
function monthStats(month){
  ...
  const variable = state.variableExpenses.filter(x=>monthOf(x.date)===month).reduce((a,b)=>a+Number(b.amount||0),0);
  const limit = Number(state.monthlyLimits[month] || Math.max(income-fixed,0));
  return {income,fixed,variable,limit,remaining:limit-variable};
}

function renderAdd(){
  const s=monthStats(selectedMonth);
  const pct=s.limit>0?Math.min(100,(s.variable/s.limit)*100):0;
  app.innerHTML=`
    <div class="grid cols-3 budget-summary-grid">
      <div class="card metric"><div class="metric-label">이번 달 사용가능 금액</div><div class="metric-value">${won(s.limit)}</div><div class="metric-foot">직접 설정하거나 수입−기본지출을 기준으로 사용</div></div>
      <div class="card metric"><div class="metric-label">현재까지 변동지출</div><div class="metric-value">${won(s.variable)}</div><div class="metric-foot">${selectedMonth} 등록 기준</div></div>
      <div class="card metric ${s.remaining>=0?'positive':'negative'}"><div class="metric-label">남은 금액</div><div class="metric-value">${won(s.remaining)}</div><div class="metric-foot">가용금액 대비 잔액</div></div>
    </div>
    <div class="grid cols-2 section-gap">
      <div class="card">
        <div class="card-head"><div><h2>변동지출 등록</h2><p>자주 쓰는 항목은 대분류 버튼으로 바로 선택할 수 있습니다.</p></div></div>
        <div class="quick-cats" id="quickCats">${state.settings.variableCategories.map((c,i)=>`<button class="chip ${i===0?'active':''}" data-cat="${esc(c)}">${esc(c)}</button>`).join('')}</div>
        <div class="divider"></div>
        <form id="expenseForm">
          <div class="form-grid">
            <div class="field"><label>대분류</label><select name="category" id="expenseCat">${state.settings.variableCategories.map(c=>`<option>${esc(c)}</option>`).join('')}</select></div>
            <div class="field" id="eventField" style="display:none"><label>이벤트 세부분류</label><select name="eventCategory">${state.settings.eventCategories.map(c=>`<option>${esc(c)}</option>`).join('')}</select></div>
            <div class="field"><label>사용금액</label><input name="amount" type="number" min="0" inputmode="numeric" placeholder="예: 35000" required></div>
            <div class="field"><label>사용날짜</label><input name="date" type="date" value="${selectedMonth===currentMonth?todayStr:selectedMonth+'-01'}" required></div>
      ...
      <div class="grid">
        <div class="card progress-card">
          <div class="card-head"><div><h2>${selectedMonth} 예산 현황</h2><p>변동지출 기준</p></div><button class="btn small" id="editLimitBtn">가용금액 설정</button></div>
          <div class="progress-row"><div class="mini-stat"><span>가용</span><strong>${won(s.limit)}</strong></div><div class="mini-stat"><span>사용</span><strong>${won(s.variable)}</strong></div><div class="mini-stat"><span>잔액</span><strong>${won(s.remaining)}</strong></div></div>
          <div class="progress-track"><div class="progress-fill" style="width:${pct}%"></div></div><div class="progress-note"><span>${pct.toFixed(0)}% 사용</span><span>${s.limit?won(s.limit-s.variable):'예산 미설정'}</span></div>
        </div>
  ...
  const cat=document.getElementById('expenseCat'), eventField=document.getElementById('eventField');
  const syncEvent=()=>eventField.style.display=cat.value==='이벤트'?'grid':'none'; syncEvent();
  cat.onchange=()=>{syncEvent(); document.querySelectorAll('#quickCats .chip').forEach(x=>x.classList.toggle('active',x.dataset.cat===cat.value));};
  document.querySelectorAll('#quickCats .chip').forEach(b=>b.onclick=()=>{cat.value=b.dataset.cat;cat.dispatchEvent(new Event('change'))});
  document.getElementById('expenseForm').onsubmit=e=>{
    e.preventDefault(); const f=new FormData(e.target); const amount=Number(f.get('amount'));
    state.variableExpenses.push({id:id(),category:f.get('category'),eventCategory:f.get('category')==='이벤트'?f.get('eventCategory'):'',amount,date:f.get('date'),memo:f.get('memo')||'',method:f.get('method')});
    saveState(); toast('변동지출을 등록했습니다.'); renderAdd();
  };
  document.getElementById('editLimitBtn').onclick=()=>{
    const v=prompt(`${selectedMonth} 변동지출 사용가능 금액을 입력하세요.`, s.limit||'');
    if(v!==null && v!==''){state.monthlyLimits[selectedMonth]=Number(v);saveState();renderAdd();}
  };
  document.getElementById('goDetails').onclick=()=>{activePage='details';render()};
}
...
function recentExpensesHtml(){
  ...
  return `<div class="summary-list">${arr.map(x=>`<div class="summary-line"><span>${esc(x.memo||x.category)}</span><div class="muted">${esc(x.date)} · ${esc(x.method)}</div><strong>${won(x.amount)}</strong></div>`).join('')}</div>`;
}
...
function renderSettings(){
  ...
  <div class="notice section-gap"><strong>공동 사용 안내</strong><br>Google Sheets 공용 데이터와 연결되어 있으며, 화면이 열릴 때와 30초마다 최신 데이터를 다시 불러옵니다.</div>`;
